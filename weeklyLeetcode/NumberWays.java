...
public class NumberWays {
    ...
    public static int helper(int startPos, int endPos, int k, int[][] dp) {
        // If startPos and endPos are equal and number of remaining steps are 0 then we
        // have an answer so return 1
        if (startPos == endPos && k == 0) {
            return 1;
        }
        // If startPos and endPos are not equal but number of remaing steps are 0 then
        // we do not have an answer so return 0
        if (k == 0) {
            return 0;
        }
